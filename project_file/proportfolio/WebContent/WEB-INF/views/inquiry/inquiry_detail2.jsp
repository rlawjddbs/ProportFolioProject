...
<style>
	...
	html{ font-size:16px; }
	body{ margin:0; }
	#inquirySubject{ margin:0; display:inline-block; vertical-align: middle; } 
	...
	$(function(){
		...
		$("#correctiveInquiry").click(function(){
			if($("#correctiveInquiry").val()=="문의 수정"){
				$(this).val("수정 완료");
				$("#taBlock").remove();
				$("#userContents").attr("disabled", null);
				var subjectTxt = $("#inquirySubject").text();
				var $tfSubject = $('<input type="text" value="'+subjectTxt+'" id="tfSubject" name="subject" />');
		...
		<article id="inquiry" style="padding-bottom:0px;">
			<form name="frm">
				<div class="headerZone">
					<h3 id="inquirySubject"><c:out value="${ inquiryDetail.subject }" /></h3>
					<p>
						...
						<span id="inquiryWriter"> | 작성자 <span class="impact"><c:out value="${ inquiryDetail.user_id }" /></span></span> |
						<span id="wDate">작성일 <span class="impact">${ inquiryDetail.w_date }</span></span>
						<span id="views"> | 조회수 <span class="impact">${ inquiryDetail.views }</span>회</span>
					</p>
					<div id="imgWrap">
				...
				<div class="description">
					...
					<textarea id="userContents" disabled="disabled" name="user_contents">${ inquiryDetail.user_contents }</textarea>
				</div>
			</form>
		</article>
		<c:choose>
			...
			<c:otherwise>
				<h3 id="answerSubject" style="margin-bottom:228px;">현재 <c:out value="${inquiryDetail.user_id }" />고객님의 문의는 답변대기 중입니다.<br />문의사항에 성심 성의껏 답변드리겠습니다.</h3>
			</c:otherwise>
		</c:choose>
		<div id="bottomZone">
